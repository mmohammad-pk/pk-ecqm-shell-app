...
module.exports = function (app) {
	...
	app.get('/login', function (req, res, next) {
		...
		apiProxy.web(req, res, {target: loginServer+'/pk-login-react',hostRewrite: 'pk-ecqm-demo.herokuapp.com'});
	});
	...
	app.all("/pk-ecqm-grid-angular/*", function(req, res) {
		...
		delete req.headers.host;
		apiProxy.web(req, res, {target: ngServer+'/pk-ecqm-grid-angular/'});
	});

	app.all("/pk-ecqm-header-vue/*", function(req, res) {
		console.log('redirecting to header : '+req.url);
		delete req.headers.host;
		apiProxy.web(req, res, {target: vueServer+'/pk-ecqm-header-vue/'});
	});
	
	app.all("/pk-bonnie-library-react/*", function(req, res) {
		console.log('redirecting to bonnie : '+req.url);
		delete req.headers.host;
		apiProxy.web(req, res, {target: reactServer+'/pk-bonnie-library-react/'});
	});
